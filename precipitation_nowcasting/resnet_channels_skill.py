...
skills=[]
lim=240*3#240
footstep=120#int(lim/24)#10
for forecast in range(20,720,120):
        pace=int(forecast/(steps*2))
        print(steps,forecast/240)
        print("##################################################")
        prange=indices[200::1000]
        print(prange)
        din=[]
        for i in range(steps):
        ...
        print("##################################################")
        res =autoencoder.evaluate(din,dout,batch_size=1)
        skills.append(np.array(res))
skills=np.array(skills)
np.save('skills2_{}.npy'.format(steps),skills)
print('DONE')
