...
with strategy.scope():
      ...
      checkpoint_path = "training/{}/cp.ckpt".format(steps)
...
for e in range(10):
        print(e)
        print("##################################################")
        for forecast in [50,10,100,20,80,200,40,120]:
        	pace=int(forecast/(steps*2))
        	...
        	print("##################################################")
        	history=autoencoder.fit(din,dout,epochs=epochs,validation_split=0.02, callbacks=[cp_callback])
