...
for i in range(1,480,4):
      ...
      fft_hr=np.mean(np.array([fft(hr[i]) for i in range(len(data))]))
      fft_0.append(fft_hr)
      phr=np.array([a[1] for a in data])
      np.save("SCREENSHOTS/PHR/{}.npy".format(i),phr) ########
      ################ Reference 
      fft_phr=np.mean(np.array([fft(phr[i]) for i in range(len(data))]))
      ...
      fft_plr=np.mean(np.array([fft(plr[i]) for i in range(len(data))]))
      ...
      print( '{}: {}: {}, {}'.format(str(i*.25), 'plr', mae_plr, acc_plr) )
      ...
      fft_li=np.mean(np.array([fft(li[i]) for i in range(len(data))]))
      ...
      print( '{}: {}: {}, {}'.format(str(i*.25), 'li', mae_li, acc_li) )
      ...
      fft_phr_1=np.mean(np.array([fft(phr_1[i]) for i in range(len(data))]))
      ...
      print( '{}: {}: {}, {}'.format(str(i*.25), 'phr_1', mae_phr_1, acc_phr_1) )
      ...
      fft_cmb=np.mean(np.array([fft(cmb[i]) for i in range(len(data))]))
      ...
      print( '{}: {}: {}, {}'.format(str(i*.25), 'cmb', mae_cmb, acc_cmb) )
