...
def process(r, i):
	try:
		a=np.load("{}/{}/PHR/{}.npy".format(folder,r,i))
		b=np.load("{}/{}/PLR/{}.npy".format(folder,r,i))
		return (a,b)
	except:
		print(r,i) 
...
with strategy.scope():   
      ...
      #layers[4.9]=tf.keras.layers.Conv2D(32, (3, 3), activation='relu', padding='same', activity_regularizer=rgl)(layers[1.9])
      ...
      for e in range(10000):
          ...
          prange=np.random.choice(range(10000,71000),1000)
          print(prange)    
          data=[process(r,0) for r in prange]
          ...
          history = model.fit(train_lr, train_hr, batch_size=10, epochs=epochs, validation_split=0.02, callbacks=[cp_callback])
