        ...
        for i,_ in enumerate(mp.run_with_snapshots(tsnapstart=tsnapstart_pr, tsnapint=tsnapint)):
            ...
            if (i%120==0): 
                ...
                if i:
                    ...
                    ############
                    for x in ['HR','PHR','PLR']:   
                        os.makedirs('EV/{}/{}'.format(i,x), exist_ok=True)
                    ############
                    print(len(qht),len(qpt),len(ql))
                    ############
                    
                    for j in range(int(year/tsnapint)):
                        np.save('EV/{}/{}/{}.npy'.format(i,'HR',j),qht[j][0] + mh.Qy[0]*mh.y)
                        np.save('EV/{}/{}/{}.npy'.format(i,'PHR',j),qpt[j][0] + mp.Qy[0]*mp.y)
                        np.save('EV/{}/{}/{}.npy'.format(i,'PLR',j),ql[j][0] + ml.Qy[0]*ml.y)
                    
        ############  
        np.save("EV/errs.npy",np.array(errs))
        plt.plot(errs)
        plt.savefig('errs.png', transparent=True)
        return 
